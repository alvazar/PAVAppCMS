...
<div class="adminOuter">
    ...
    <div>
        <a href="/admin/builder/data/detail"><button>Добавить блок данных</button></a>
    </div>
    <div style="height: 20px;"></div>
    ...
    <div class="main">
        ...
        <div class="actionList" id="list_items">
            <div style="grid-template-columns: 40px 63px 300px 280px 50px;">
                <div>ID</div>
                <div>Ключ</div>
                <div>Название</div>
...
const templates = {
    item: `<div style="grid-template-columns: 40px 63px 300px 280px 50px;">
        <div>{ID}</div>
        <div>{dataKey}</div>
        <div>{name}</div>
        <div>{dateModify}</div>
        <div><a href="/admin/builder/data/detail?ID={ID}" title="Редактировать страницу"><svg class="adminIcon">
