...
if (!empty($_POST['action'])) {
    ...
    if ($action === 'getList') {
        ...
        // prepare list
        $productModel = $Site->model('DB\Product');
        foreach ($response['items'] as $key => $item) {
            $item['info'] = '';
            if (!empty($item['templateData']['product_id'])) {
                $product = $productModel->getList([
                    'select' => ['product_name'],
                    'where' => ['product_id' => $item['templateData']['product_id']]
                ])[0] ?? [];
                if (!empty($product)) {
                    $item['info'] .= sprintf(
                        '<b>Продукт:</b> [%1$d] <a href="/admin/product/%1$d" target="_blank">%2$s</a><br>',
                        $item['templateData']['product_id'],
